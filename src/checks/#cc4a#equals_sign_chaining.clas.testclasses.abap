CLASS ltcl_test DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      run_test_check_findings FOR TESTING RAISING cx_static_check.
...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD run_test_check_findings.
    ...
    DATA(finding_1_obj) = cl_ci_atc_unit_driver=>get_method_object( VALUE #( class  = co_test_object-object_name
                                                                             method = co_test_method_name-finding_1 ) ).
    DATA(finding_1_loc) = VALUE if_ci_atc_check=>ty_location( object   = finding_1_obj
                                                          position = VALUE #( line   = 7
                                                                              column = 4 ) ).
    DATA(qf1_finding_1) = VALUE if_ci_atc_unit_asserter=>ty_expected_quickfixes( ( code          = VALUE #( ( |C = D.| )
                                                                                                            ( |B = C.| )
                                                                                                            ( |A = B.| ) )
                                                                                   location      = finding_1_loc
                                                                                   quickfix_code = /cc4a/equals_sign_chaining=>quickfix_codes-break_chain ) ).
    DATA(finding_2_obj) = cl_ci_atc_unit_driver=>get_method_object( VALUE #( class  = co_test_object-object_name
                                                                             method = co_test_method_name-finding_2 ) ).
    DATA(finding_2_loc) = VALUE if_ci_atc_check=>ty_location( object   = finding_2_obj
                                                          position = VALUE #( line   = 6
                                                                              column = 4 ) ).
    DATA(qf1_finding_2) = VALUE if_ci_atc_unit_asserter=>ty_expected_quickfixes( ( code          = VALUE #( ( |B = CONV #( c ).| )
                                                                                                            ( |A = B.| ) )
                                                                                   location      = finding_1_loc
                                                                                   quickfix_code = /cc4a/equals_sign_chaining=>quickfix_codes-break_chain ) ).
    " Finding 3 should not appear due to the pseudo comment
    DATA(finding_4_obj) = cl_ci_atc_unit_driver=>get_method_object( VALUE #( class  = co_test_object-object_name
                                                                             method = co_test_method_name-finding_4 ) ).
    DATA(finding_4_loc) = VALUE if_ci_atc_check=>ty_location( object   = finding_4_obj
                                                          position = VALUE #( line   = 7
                                                                              column = 6 ) ).
    DATA(qf1_finding_4) = VALUE if_ci_atc_unit_asserter=>ty_expected_quickfixes( ( code          = VALUE #( ( |B = C.| )
                                                                                                            ( |A = B.| ) )
