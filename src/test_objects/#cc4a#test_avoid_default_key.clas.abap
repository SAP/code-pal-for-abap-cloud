class /cc4a/test_avoid_default_key definition
  ...
  private section.
    ...
    class-data abc type table of tadir with default key.
    class-data def type table of tadir with empty key.
    class-data ghi type table of tadir with default key. "#EC DEFAULT_KEY
endclass.
...
class /cc4a/test_avoid_default_key implementation.
  ...
  method with_pseudo_comments.
    data mno type table of tadir with empty key.
    data pqr type table of tadir with empty key.
    data stu type table of tadir with default key.     "#EC DEFAULT_KEY

    types: begin of type1,
             vwx type standard table of tadir with non-unique key object,
  ...
  method without_pseudo_comments.
    data mno type table of tadir with empty key.
    data pqr type table of tadir with empty key.
    data stu type table of tadir with default key.
